  ...
  var Parser = class {
    ...
    parseStatement() {
      ...
      if (this.check("IDENTIFIER" /* IDENTIFIER */)) {
        const isDecl = this.peek(1).type === "COLON" /* COLON */;
        if (isDecl) {
    ...
    parseFunctionDeclaration(isAlreadyParsedFunctionKeyword = false) {
      ...
      this.consume("COLON" /* COLON */, "Expected ':' after function parameters for return type.");
...
`;
  ...
  if (typeof window !== "undefined") {
    window.Denner = { Lexer, Parser, JSCodeGenerator };
  }
